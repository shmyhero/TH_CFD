...
- (void) showScene:(NSString *)sceneName
{
	...
	if([sceneName isEqualToString:@"MeiQia"]) {
		...
		dispatch_async(queue, ^{
			...
			BOOL isLive = [StockDataManager sharedInstance].isLive;
			style.navBarColor = isLive ? [UIColor colorWithHexWithLong:0x425a85] : [UIColor colorWithHexWithLong:0x1962dd];
			...
			style.backgroundColor = [UIColor colorWithHexWithLong:0xf0f0f0];
			style.incomingBubbleColor = [UIColor whiteColor];
			style.incomingMsgTextColor = [UIColor blackColor];
			style.outgoingBubbleColor = [UIColor colorWithHexWithLong:0xcef2ff];
